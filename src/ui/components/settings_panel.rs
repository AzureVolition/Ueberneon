...
pub fn SettingsPanel(
    ...
    let mut do_refresh = {
        move |inst_id: String| {
            // 查找实例对应的 provider_id
            let inst =
                crate::db::with_db(|conn| provider_instance::get(conn, &inst_id).ok().flatten());
            ...
            let key2 = key.clone();
            spawn(async move {
                let prov = crate::db::with_db(|conn| provider::get(conn, &pid2).ok().flatten());
                match prov {
                    Some(p) => {
    ...
    let mut do_delete = {
        move |inst_id: String| {
            crate::db::with_db(|conn| {
                if let Err(e) = provider_instance::delete(conn, &inst_id) {
                    tracing::error!(target:"db", error=%e, "delete provider instance");
                }
            });
    ...
    let mut do_save_key = {
        move |inst_id: String| {
            ...
            crate::db::with_db(|conn| {
                if let Err(e) = provider_instance::update_key(conn, &inst_id, &encoded) {
                    tracing::error!(target:"db", error=%e, "update provider key");
                }
                conn.execute(
                ...
                match crate::db::model_fetch::fetch_models(&prov_row, &raw_key).await {
                    ...
                    Ok(models) => {
                        ...
                        let write_result = crate::db::with_db_result(|conn| {
                            ...
                            provider_instance::insert(conn, &row).map_err(|e| format!("db: {e}"))
                        });
                        match write_result {
                            Err(e) => {
                                ...
                                match crate::db::with_db(|conn| provider_instance::list_all(conn)) {
                                    ...
                                    Err(e) => {
                                        err_sig.set(Some(format!(
                                            "saved but failed to refresh list: {e}"
                                        )));
                ...
                match crate::db::model_fetch::fetch_models(&prov_row, &raw_key).await {
                    ...
                    Ok(models) => {
                        ...
                        match write_result {
                            ...
                            Ok(()) => {
                                providers_cache_sig.set(crate::db::with_db(|conn| {
    ...
    let density_options = [("comfortable", "comfortable"), ("compact", "compact")];

    // ── General 信号（match 外） ──
    let mut current_agent_id =
        use_signal(|| settings::get().general.default_agent_config_id.clone());
    let mut current_action_mode =
        use_signal(|| settings::get().general.default_action_mode.clone());
    let mut current_agent_mode = use_signal(|| settings::get().general.default_agent_mode.clone());

    rsx! {
        if tab == SettingsTab::Skills {
            SkillsPanel { project_path: project_path.clone() }
            } else {
