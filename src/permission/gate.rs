...
use crate::permission::{Check, Decision, bash_decompose, extract_subjects};
    ...
    pub fn check(
        ...
        match decision {
            ...
            Decision::Ask => {
                ...
                Err(BlockedReason::NeedsApproval(format!(
                    "tool {} ({}) needs approval",
                    tool, subject
                )))
            }
            Decision::Deny(_) => {
                let subject = subjects.first().map(|s| s.as_str()).unwrap_or("");
                Err(BlockedReason::Denied(format!(
        ...
        match decision {
            ...
            Decision::Ask => {
                ...
                match approver.approve(&ctx).await {
                    Ok(true) => Ok(()),
                    Ok(false) => Err(BlockedReason::Denied("user declined this tool call".into())),
    ...
    pub fn check_with_mode(
        ...
        match agent_mode {
            ...
            AgentMode::Cautious => {
                ...
                let result = self.check(tool, args, read_only);
                match result {
                    Ok(()) if !read_only => Err(BlockedReason::NeedsApproval(format!(
                        "cautious mode: {} needs approval",
                        tool
                    ))),
                    other => other,
                }
...
pub trait PermissionChecked {
    ...
    fn agent_mode_enabled(&self) -> bool {
        true
    }
...
mod tests {
    ...
    fn policy_writer_fallback_deny() {
        ...
        assert_eq!(
            p.evaluate("edit_file", "whatever", false),
    ...
    fn policy_with_real_checks() {
        let p = Policy::new(
            Decision::Ask,
            vec![Box::new(DenySystemPaths), Box::new(ForcePushGuard)],
        );
        assert_eq!(
            p.evaluate("edit_file", "/etc/passwd", false),
            Decision::Deny("denied".into())
        );
    ...
    fn policy_subjects_combine() {
        let p = Policy::new(
            Decision::Allow,
            vec![Box::new(MockCheck::deny("move_file", "/etc/*"))],
        );
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/a.txt".into(), "/etc/b.txt".into()],
                false
            ),
            Decision::Deny("denied".into())
        );
    ...
    fn policy_subjects_ask_wins_over_allow() {
        let p = Policy::new(
            Decision::Allow,
            vec![Box::new(MockCheck::ask("move_file", "/secret/*"))],
        );
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/a.txt".into(), "/secret/b.txt".into()],
                false
            ),
            Decision::Ask
        );
    ...
    fn gate_denies_system_path() {
        let p = Policy::new(Decision::Ask, vec![Box::new(DenySystemPaths)]);
        let g = Gate::new(p);
    ...
    fn gate_ask_falls_back_to_allow_noninteractive() {
        let p = Policy::new(Decision::Ask, vec![Box::new(MockCheck::ask("bash", "*"))]);
    ...
    impl MockCheck {
        ...
        fn deny(tool: &'static str, pattern: &'static str) -> Self {
            Self {
                tool,
                pattern,
                decision: Decision::Deny("denied".into()),
            }
        }
    }
    ...
    fn permission_checked_with_deny_system() {
        ...
        assert!(matches!(
            t.check_permission("edit_file", &args, AgentMode::Ask),
            Decision::Allow
        ));
    }
    ...
    fn policy_integration_mixed_checks() {
        ...
        assert_eq!(
            p.evaluate("write_file", "/ok/path.txt", false),
            Decision::Ask
        );
        // DenySystemPaths 对任何工具的 /etc 路径都匹配
        assert_eq!(
            p.evaluate("read_file", "/etc/passwd", true),
            Decision::Deny("denied".into())
        );
        // 安全路径的 read_file → Allow
        assert_eq!(
            p.evaluate("read_file", "/home/user/file.txt", true),
            Decision::Allow
        );
    }
    ...
    fn gate_noninteractive_ask_fallback() {
        let p = Policy::new(Decision::Ask, vec![Box::new(ForcePushGuard)]);
        let g = Gate::new(p);
    ...
    fn decision_combine_integration() {
        ...
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/a.txt".into(), "/etc/passwd".into()],
                false
            ),
            Decision::Deny("denied".into())
        );
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/a.txt".into(), "/secret/data".into()],
                false
            ),
            Decision::Ask
        );
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/a.txt".into(), "/home/b.txt".into()],
                false
            ),
            Decision::Allow
        );
    ...
    fn danger_and_deny_independent() {
        let p = Policy::new(Decision::Ask, vec![Box::new(DangerousPatternDetector)]);
        ...
        assert_eq!(
            p2.evaluate("edit_file", "/etc/passwd", false),
            Decision::Deny("denied".into())
        );
    ...
    fn permission_checked_unrestrained_asks_become_allow() {
        ...
        // Ask mode → 需要询问
        assert!(!matches!(
            t.check_permission("bash", &args, AgentMode::Ask),
            Decision::Allow
        ));
        // Unrestrained mode → 放行（Ask 降级为 Allow）
        assert!(matches!(
            t.check_permission("bash", &args, AgentMode::Unrestrained),
            Decision::Allow
        ));
    }
    ...
    fn permission_checked_cautious_mode_asks_writers() {
        ...
        assert!(!matches!(
            t.check_permission("edit_file", &args, AgentMode::Cautious),
            Decision::Allow
        ));
        // Cautious mode + 只读工具 → 不触发
        assert!(matches!(
            t.check_permission("read_file", &args, AgentMode::Cautious),
            Decision::Allow
        ));
    }
    ...
    fn gate_check_with_mode_deny_still_works() {
        let p = Policy::new(Decision::Allow, vec![Box::new(DenySystemPaths)]);
        let g = Gate::new(p);
        let args = serde_json::json!({"file_path": "/etc/passwd"});
    ...
    const TEMP_PATH_PREFIXES: &[&str] = &["/tmp/", "/var/tmp/", "/private/tmp/"];
    ...
    fn policy_with_safe_directory_guard() {
        let p = Policy::new(
            Decision::Ask,
            vec![Box::new(SafeDirectoryGuard), Box::new(DenySystemPaths)],
        );

        // 临时路径 → Deny
        assert_eq!(
            p.evaluate("write_file", "/tmp/x.txt", false),
            Decision::Deny("denied".into())
        );
        // 系统路径 → Deny（来自 DenySystemPaths）
        assert_eq!(
            p.evaluate("edit_file", "/etc/config", false),
            Decision::Deny("denied".into())
        );
        // 项目路径 → Allow（来自 SafeDirectoryGuard）
        assert_eq!(
            p.evaluate("edit_file", "/home/user/project/main.rs", false),
            Decision::Allow
        );
        // 未知路径 → fallback Ask
        assert_eq!(
            p.evaluate("write_file", "/home/user/other.txt", false),
            Decision::Ask
        );
        // 只读操作 → Allow
    ...
    fn gate_with_safe_directory_guard_full_flow() {
        let p = Policy::new(Decision::Ask, vec![Box::new(SafeDirectoryGuard)]);
        let g = Gate::new(p);
    ...
    fn permission_checked_with_safe_directory_guard() {
        ...
        let editor = Editor {
            checks: vec![Box::new(SafeDirectoryGuard), Box::new(DenySystemPaths)],
        };
        ...
        let args = serde_json::json!({"file_path": "/home/user/project/main.rs"});
        assert!(matches!(
            editor.check_permission("edit_file", &args, AgentMode::Ask),
            Decision::Allow
        ));
        ...
        let args = serde_json::json!({"file_path": "/srv/app/config.json"});
        assert!(!matches!(
            editor.check_permission("edit_file", &args, AgentMode::Cautious),
            Decision::Allow
        ));
        ...
        let args = serde_json::json!({"file_path": "/srv/app/config.json"});
        assert!(matches!(
            editor.check_permission("edit_file", &args, AgentMode::Unrestrained),
            Decision::Allow
        ));
    }
    ...
    fn safe_directory_guard_with_move_file() {
        ...
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/home/user/project/a.txt".into(), "/tmp/a.txt".into(),],
                false
            ),
            Decision::Deny("denied".into())
        );
        ...
        assert_eq!(
            p.evaluate_subjects(
                "move_file",
                &["/tmp/a.txt".into(), "/home/user/project/a.txt".into(),],
                false
            ),
            Decision::Deny("denied".into())
        );
    ...
    fn compound_simple_command_falls_back() {
        ...
        let p = Policy::new(Decision::Ask, vec![Box::new(ReadOnlyBashClassifier)]);
        assert_eq!(
            p.evaluate_compound("bash", "ls -la", false),
            Decision::Allow
        );
        assert_eq!(
            p.evaluate_compound("bash", "rm -rf /", false),
            Decision::Ask
        );
    }
    ...
    fn compound_force_push_denied() {
        ...
        let p = Policy::new(
            Decision::Ask,
            vec![Box::new(DenyForcePush), Box::new(ReadOnlyBashClassifier)],
        );
        assert_eq!(
            p.evaluate_compound("bash", "git status && git push --force origin main", false),
            Decision::Deny("denied".into())
    ...
    fn gate_compound_readonly_then_force_push() {
        let p = Policy::new(
            Decision::Ask,
            vec![Box::new(ReadOnlyBashClassifier), Box::new(ForcePushGuard)],
        );
        let g = Gate::new(p);
    ...
    fn gate_compound_partial_deny() {
        ...
        let p = Policy::new(Decision::Ask, vec![Box::new(DenyRmRf)]);
        let g = Gate::new(p);
